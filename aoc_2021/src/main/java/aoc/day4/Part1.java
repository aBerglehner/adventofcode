...
//            Helper.prettyPrintDoubleDeepList(matrixContainer);
            ...
            BingoResult firstBingoMatrix = getFirstBingoMatrix(bingoNumbers, matrixContainer);
//            System.out.println("firstBingoMatrix = " + firstBingoMatrix);
            int sum = getSum(firstBingoMatrix);
            System.out.println("sum = " + sum);
            Integer lastBingoNumber = firstBingoMatrix.bingoNumbers.get(firstBingoMatrix.bingoNumbers.size() - 1);
    ...
    private static BingoResult getFirstBingoMatrix(List<Integer> bingoNumbers, List<List<List<Integer>>> matrixContainer) {
        int start = 5;
        int end = bingoNumbers.size() - 1;
        while (start <= end) {
            List<Integer> curBingoNumbers = bingoNumbers.stream().limit(start).toList();
            for (List<List<Integer>> curMatrix : matrixContainer) {

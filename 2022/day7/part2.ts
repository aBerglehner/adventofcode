...
const totalSumFoldersValuesSorted: number[] = Object.values(totalSumFolders).sort(
    (a, b) => b - a
);

const maxSpaceAllowed: number = 70_000_000 - 30_000_000;
const spaceNeeded: number = Math.abs(maxSpaceAllowed - totalSumFolders['/.']);
console.log('max space allowed: ', maxSpaceAllowed);
console.log('need to delete: ', spaceNeeded);

for (let i = 0; i < totalSumFoldersValuesSorted.length; i += 1) {
    if (totalSumFoldersValuesSorted[i] < spaceNeeded) {
        console.log('result: ', totalSumFoldersValuesSorted[i - 1]);
        break;
    }
}

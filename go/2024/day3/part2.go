...
func main() {
	...
	for _, line := range lines {
		...
		for {
			...
			lastDo := strings.LastIndex(line[:startPos], "do()")
			lastDont := strings.LastIndex(line[:startPos], "don't()")
			if lastDo > lastDont {
				mulEnabled = true
			}
			if lastDont > lastDo {
			...
			// if count != 0 && lastDo == -1 && lastDont == -1 {
			// 	mulEnabled = false
			// }
			fmt.Println("lastDo: ", lastDo)
			fmt.Println("lastDont: ", lastDont)
			fmt.Println("mulEnabled: ", mulEnabled)
			if mulEnabled {
